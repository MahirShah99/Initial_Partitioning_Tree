...
def least_allocated_alocation(loc):
	"""
...
def getMedian(cur, table, attribute, primary_column):
	"""
...
if __name__ == "__main__":
	...
	no_of_bucket = int(input("Enter Number of Bucket (power of 2):- "))
	...
	getAttributeList(cur, table_name, no_of_bucket)

	# print(attribute_allocation_dict)
	...
	for depth in range(max_depth):
		if(depth==1):
			...
			loc=0
		attribute_name = least_allocated_alocation(loc)
	...
	for i in range(no_of_bucket-1, 2*no_of_bucket-1):
		partition_list.append(getString(i, int(ceil(i/2)-1) ) )	

	# print(partition_list)


	file = open(table_name+"_KDD_partition.txt", "w")
	file.write(str(partition_list))
	file.close()

	part_cnt = 1
	file = open(table_name+"_KDD_partition2.txt", "w")
	...
	for i in range(1, len(partition_list)+1):
		tmp_table = table_name+"_kdd_p_"+str(i)
